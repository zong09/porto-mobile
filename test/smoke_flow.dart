// T4.2 — end-to-end smoke flow (Phase 4). THE FLOW ONLY — no `main()`.
//
// Two entry points share this single body so they cannot drift apart:
//   * `test/smoke_test.dart`          — headless, runs under `flutter test`.
//   * `integration_test/app_test.dart` — on-device, needs an emulator/device.
//
// Deliberately NOT named `*_test.dart`: `flutter test` collects only that
// pattern, so this file is never run directly despite living in `test/`.
//
// It drives the *real* app widget (PortoApp) over the *real* provider graph and
// a *real* on-disk SQLite DB — only the two price clients are mocked, so the
// price-cache persistence + offline fallback chain is genuinely exercised.
//
// Flow:
...
/// Registers the smoke-flow test. The caller supplies the binding.
void runSmokeFlow() {
